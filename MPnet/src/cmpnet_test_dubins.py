...
import os
import torch
from gem_eval_dubins import eval_tasks
import plan_s2d, plan_c2d, plan_r3d, plan_r2d, plan_dubins
...
def main(args):
    ...
    if args.env_type == 's2d':
        ...
        IsInCollision = plan_dubins.IsInCollision
    ...
    print('loading...')
    #  Load the set of seed values from the train data set
    seen_data = []
    files = data_loader_dubins.get_files(
        os.path.join('data', 'RRT_star_planning_prune'))
    ...
    # Load a set of seed values from the test data set
    unseen_data = []
    files = data_loader_dubins.get_files(
        os.path.join('data', 'RRT_star_planning_prune2'))
    for i, f_i in enumerate(files):
        s = int(re.findall(r'\d+', f_i)[0])
        unseen_data.append(s)
        if i == args.unseen_N - 1:
            break
    ...
    for _ in range(T):
        ...
        fes_path_, valid_path_ = eval_tasks(mpNet, unseen_data, time_file,
                                            IsInCollision, normalize_func,
                                            unnormalize_func, True)
        valid_path = valid_path_.flatten()
        fes_path = fes_path_.flatten(
