...
class MPnetTrain(MPnetBase):
    ...
    def train(self, numEnvs, numPaths, trainDataPath, testDataPath):
        ...
        obs, inputs, targets = self.load_dataset(N=numEnvs * numPaths,
                                                 logdir=trainDataPath)
        trainInput, trainTarget = self.format_data(obs, inputs, targets)
        # NOTE: Change the number of test cases for the path
        obs_test, inputs_test, targets_test = self.load_dataset(
            N=4000, logdir=testDataPath)
        testInput, testTarget = self.format_data(obs_test, inputs_test,
                                                 targets_test)
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range((numPaths * numEnvs) // self.batchSize):
                sample_index = indices[i * self.batchSize:(i + 1) *
                                       self.batchSize]
                bi, bt = trainInput[sample_index, :], trainTarget[
                    sample_index, :]
                # Run gradient descent
                self.mpNet.zero_grad()
                self.mpNet.observe(bi, 0, bt, False)
                num_path_trained += 1
            loss = get_numpy(
                self.mpNet.loss(self.mpNet(trainInput), trainTarget))
            print('Epoch {} - train loss: {}'.format(epoch, loss))
            train_loss.append(loss)
            loss = get_numpy(self.mpNet.loss(self.mpNet(testInput),
                                             testTarget))
