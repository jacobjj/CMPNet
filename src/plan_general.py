...
def neural_replan(mpNet,
    ...
    for i in range(len(path) - 1):
        ...
        start = path[i]
        goal = path[i + 1]
        # If the steerTo function is passed as argument, then use it.
        if not steerTo:
        ...
        if steer:
            ...
            if mini_path:
                new_path += removeCollision(
                    mini_path[1:], obc, IsInCollision)  # take out start point
            else:
...
def complete_replan_global(mpNet, path, true_path, true_path_length, obc, obs, obs_i, \
    ...
    dataset, targets, env_indices = transformToTrain(demo_path, len(demo_path),
                                                     obs, obs_i)
    added_data = list(zip(dataset, targets, env_indices))
    bi = np.concatenate(
        (obs.numpy().reshape(1, -1).repeat(len(dataset), axis=0), dataset),
    ...
    demo_path = [
        torch.from_numpy(p).type(torch.FloatTensor) for p in demo_path
    ]
...
def transformToTrain(path, path_length, obs, obs_i):
    ...
    for m in range(0, path_length - 1):
        data = np.concatenate(
            (path[m], path[path_length - 1])).astype(np.float32)
